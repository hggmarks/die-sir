...
pub fn eval(expr: Node) -> Result<EvalResult, Box<dyn error::Error>> {
    match expr {
        ...
        Node::Negative(expr1) => {
            ...
            match value {
                EvalResult::Number(val) => Ok(EvalResult::Number(-val)),
        ...
        Node::Die(expr1, expr2) => {
            ...
            if let (EvalResult::Number(num_rolls), EvalResult::Number(num_sides)) =
                (num_rolls, num_sides)
            {
                if num_rolls == 0.0 || num_sides == 0.0 {
                    return Ok(EvalResult::Number(0.0));
                } else if num_rolls == 1.0 {
                    return Ok(EvalResult::Number(
                        rng.gen_range(1..=(num_sides as i128)) as f64
                    ));
                }

                for _ in 0..(num_rolls as i128) {
